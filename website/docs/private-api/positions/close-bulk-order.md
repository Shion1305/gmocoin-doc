...
Closes bulk orders. Target: Margin trading
...
| --- | --- | --- | --- |
| symbol | string | Required | The handling margin symbols are [here](https://api.coin.z.com/docs/#symbols) |
| side | string | Required | `BUY` `SELL` |
| executionType | string | Required | `MARKET` `LIMIT` `STOP` |
| timeInForce | string | Optional | `FAK` `FAS` `FOK` (`SOK` is Post-only order) `LIMIT` can only be specified *If timeInForce is not specified, FAK for MARKET and STOP, FAS for LIMIT. Please refer to [here](https://api.coin.z.com/docs/#symbols) for symbols for which SOK can be specified. |
| price | string | *Depending on executionType | Required if `LIMIT` `STOP`. Not required if `MARKET`. |
